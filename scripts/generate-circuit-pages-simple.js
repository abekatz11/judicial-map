...
const circuitsGeoJSON = JSON.parse(await readFile("src/data/circuits.json", "utf-8"));
...
const recentCases = caseData.recentCases.filter(c => c.circuit === "${circuit.name}");
const citedCases = caseData.mostCitedCases.filter(c => c.circuit === "${circuit.name}");
\`\`\`
...
for (const circuit of circuits) {
  await writeFile(`src/circuits/${circuit.slug}.md`, template(circuit));
}

console.log(`✅ Regenerated ${circuits.length} circuit pages`);
