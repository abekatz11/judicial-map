...
const circuitFeature = circuitsGeoJSON.features.find(f =>
  f.properties.Name === "Eleventh Circuit"
);
```
...
const recentCases = caseData.recentCases.filter(c => c.circuit === "Eleventh Circuit");
const citedCases = caseData.mostCitedCases.filter(c => c.circuit === "Eleventh Circuit");
```
